## Day 1
- Installed Flutter SDK
- Setup Xcode and Android Studio
- Ran app on emulator

## Day 2
- Added “Next” button to the main screen
- Connected button to update random word
- Demo: [Watch video](videos/day2-next-button.mov)
- PR: [Add “Next” button to generate new random word](https://github.com/psreddyio/my_awesome_namer/pull/1)

## Day 3
– Improve UI, styling, and accessibility
- Refactored UI with a custom BigCard widget, applied theming, improved text styling, centered layout, and added accessibility support for screen readers
Demo: [Watch video](videos/day3-make-app-prettier.mov)
- PR: 
